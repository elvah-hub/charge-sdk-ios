...
struct ChargeProviderTests {
  ...
  func sharedSessionUpdatesWithMultipleSubscriptions() async throws {
    let subscriptionManager = ChargeProvider.SubscriptionManager()
    let authentication = ChargeAuthentication(token: "token", expiryDate: nil)
  ...
  @Test("Shared session update polling restart after subscription removal") @available(iOS 16.0, *) func sharedSessionUpdatePollingRestart() async throws {
    let subscriptionManager = ChargeProvider.SubscriptionManager()
    let authentication = ChargeAuthentication(token: "token", expiryDate: nil)
  ...
  @Test("Shared session updates with many concurrent subscriptions") @available(iOS 16.0, *) func sharedSessionUpdatesSpam() async throws {
    let subscriptionManager = ChargeProvider.SubscriptionManager()
    let authentication = ChargeAuthentication(token: "token", expiryDate: nil)
  ...
  @Test("Shared session updates handles network failures and recovery") @available(iOS 16.0, *) func sharedSessionUpdatesWithBasicFailure() async throws {
    let subscriptionManager = ChargeProvider.SubscriptionManager()
    let authentication = ChargeAuthentication(token: "token", expiryDate: nil)
