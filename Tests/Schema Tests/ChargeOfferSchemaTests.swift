...
/// Tests for ChargeOffer schema parsing functionality.
@Suite("ChargeOffer Schema Parsing Tests", .tags(.parsing))
struct ChargeOfferSchemaTests {
  ...
  func chargeOfferInvalidOfferType() throws {
    ...
    let schema = try SchemaTestHelpers.decodeSchema(invalidTypeJSON, as: ChargeOfferSchema.self)
  ...
  func chargeOfferComplexPricing() throws {
    ...
    // When: Parsing the schema
    let schema = try SchemaTestHelpers.decodeSchema(complexPricingJSON, as: ChargeOfferSchema.self)
    let chargeOffer = try ChargeOffer.parse(schema, in: mockSite)
