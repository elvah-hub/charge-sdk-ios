...
extension SchemaParsingTests {
	...
	) func chargePointValidDCPowerSpecification(
		powerTypeString: String,
		powerType: PowerType
	) throws {
		// Given: Valid charge offer schema with DC power specification
		let validJSON = """
		{
	...
	func chargePointInvalidPowerType() throws {
		...
		// When: Parsing the schema
		let schema = try decodeSchema(invalidPowerTypeJSON, as: ChargeOfferSchema.self)
		let chargePoint = try ChargePoint.parse(schema)
