...
struct ChargePriceSchemaTests {
  ...
  func chargePriceSchemaParses() throws {
    ...
    let schema = try SchemaTestHelpers.decodeSchema(json, as: ChargePriceSchema.self)
  ...
  func chargePriceSchemaHandlesOptionalFields() throws {
    ...
    let schema = try SchemaTestHelpers.decodeSchema(jsonWithoutOptionals, as: ChargePriceSchema.self)
    let chargePrice = try ChargePrice.parse(schema)
  ...
  func chargePriceSchemaHandlesPartialBlockingFee() throws {
    ...
    let schema = try SchemaTestHelpers.decodeSchema(jsonWithPartialBlockingFee, as: ChargePriceSchema.self)
    let chargePrice = try ChargePrice.parse(schema)
