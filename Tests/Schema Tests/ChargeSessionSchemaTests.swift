...
extension SchemaParsingTests {
	@Test("ChargeSession schema parses valid JSON correctly") func chargeSessionValidParsing() throws {
		...
		let schema = try decodeSchema(validJSON, as: ChargeSessionSchema.self)
	...
	func chargeSessionOptionalFields() throws {
		...
		// When: Parsing the schema
		let schema = try decodeSchema(minimalJSON, as: ChargeSessionSchema.self)
		let chargeSession = try ChargeSession.parse(schema)
	...
	func chargeSessionInvalidStatus() throws {
		...
		let schema = try decodeSchema(invalidStatusJSON, as: ChargeSessionSchema.self)
	...
	@Test(
		...
		// When: Parsing the schema
		let schema = try decodeSchema(json, as: ChargeSessionSchema.self)
		let chargeSession = try ChargeSession.parse(schema)
