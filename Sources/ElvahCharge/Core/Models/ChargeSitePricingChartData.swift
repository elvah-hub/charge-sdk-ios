...
public struct ChargeSitePricingChartData: Hashable, Sendable {
  ...
  /// The base price (usually the day's lowest price) used for the baseline band.
...
package extension ChargeSitePricingSchedule {
  ...
  func makeChart(
    ...
    let entry: Entry? = {
      if calendar.isDateInToday(day) { return dailyPricing.today }
      if calendar.isDateInYesterday(day) { return dailyPricing.yesterday }
      if calendar.isDateInTomorrow(day) { return dailyPricing.tomorrow }
