...
package extension DailyPricingView {
  ...
  struct PriceChart: View {
    ...
    package var body: some View {
      ...
      .chartYAxis(.hidden)
      .chartXScale(domain: DailyPricingComputation.fullDayDomain(for: data.day))
    ...
    private func currentTimeMarker(now: Date) -> some ChartContent {
      let price = DailyPricingComputation.currentPrice(at: now, in: data)
      let isDiscount = DailyPricingComputation.isDiscounted(at: now, in: data)
    ...
    private func updateSelection(from location: CGPoint, proxy: ChartProxy, geometry: GeometryProxy) {
      ...
      if let tapped: Date = proxy.value(atX: plotLocation.x, as: Date.self) {
        ...
        if let current = selectedMoment,
           let currentRange = DailyPricingComputation.segmentRange(containing: current, in: data),
           let tappedRange = DailyPricingComputation.segmentRange(containing: tapped, in: data),
    ...
    private func opacityForMark(in start: Date, to end: Date) -> Double {
      ...
      let domain = DailyPricingComputation.fullDayDomain(for: data.day)
...
#Preview("PriceChart (Today)") {
  DailyPricingView.PriceChart(data: PricingSchedule.mock.chartData()[1], selectedMoment: .constant(nil))
    .frame(height: 180)
    .padding()
